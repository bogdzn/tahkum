...
Test(initial_setup, basicparser)
{
    ...
    cr_assert_eq(settings.max_retries, DEFAULT_MAX_RETRIES);
    cr_assert_eq(settings.wait, DEFAULT_WAIT_TIME);
    cr_assert_eq(settings.fake_socket, false);
    cr_assert_eq(settings.filepath, 0);
}
...
Test(get_user_commands, getusrcmd)
{
    ...
    cr_assert_str_eq(*second, "down 50");
    cr_assert_str_eq(*third, "up 50");
